...
class FrameInfoRetrieval:
    ...
    def get_comment(self, node: str) -> np.ndarray:
        """ Retrieve comment (description of frame) """
        return self.linguistic_kg.get_comment_node(node)

    def get_frame_element(self, node: str) -> np.ndarray:
    ...
    def __call__(self, nodes: List[str], cached: dict) -> dict:
        """ Main 
        nodes: frames
        """
        res = {}
        for i in tqdm(range(len(nodes))):
            node = nodes[i]
            frame_elements, types_fe, comments = self.get_frame_element(node=node)
